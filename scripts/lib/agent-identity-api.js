...
export class AgentIdentityAPIClient {
     ...
     */
    handleAxiosError(error, context, requestBody) {
        if (error.response) {
    ...
    async registerAgent(request) {
        try {
            const response = await axios.post(`${this.baseUrl}/workload-principals/api/v1/ai-agents`, request, this.getAxiosConfig());
     ...
     */
    async pollOperation(operationUrl, timeoutMs = 60000, intervalMs = 2000) {
    ...
    async getAgent(agentId) {
        try {
            const response = await axios.get(`${this.baseUrl}/workload-principals/api/v1/ai-agents/${agentId}`, this.getAxiosConfig());
    ...
    async activateAgent(agentId) {
        try {
            const response = await axios.post(`${this.baseUrl}/workload-principals/api/v1/ai-agents/${agentId}/lifecycle/activate`, {}, this.getAxiosConfig());
            if (response.status !== 202) {
                throw new Error(`Unexpected status: ${response.status}`);
            }
            const operationUrl = response.headers['location'];
    ...
    async deactivateAgent(agentId) {
        try {
            const response = await axios.post(`${this.baseUrl}/workload-principals/api/v1/ai-agents/${agentId}/lifecycle/deactivate`, {}, this.getAxiosConfig());
            if (response.status !== 202) {
                throw new Error(`Unexpected status: ${response.status}`);
            }
            const operationUrl = response.headers['location'];
    ...
    async deleteAgent(agentId) {
        try {
            const response = await axios.delete(`${this.baseUrl}/workload-principals/api/v1/ai-agents/${agentId}`, this.getAxiosConfig());
            if (response.status !== 202) {
                throw new Error(`Unexpected status: ${response.status}`);
            }
            const operationUrl = response.headers['location'];
     ...
     */
    async uploadPublicKey(agentId, jwk) {
        try {
            const response = await axios.post(`${this.baseUrl}/workload-principals/api/v1/ai-agents/${agentId}/credentials/jwks`, jwk, this.getAxiosConfig());
     ...
     */
    async createConnection(agentId, request) {
        try {
            const response = await axios.post(`${this.baseUrl}/workload-principals/api/v1/ai-agents/${agentId}/connections`, request, this.getAxiosConfig());
            return response.data;
        }
        catch (error) {
     ...
     */
    async deleteConnection(agentId, connectionId) {
        try {
            const response = await axios.delete(`${this.baseUrl}/workload-principals/api/v1/ai-agents/${agentId}/connections/${connectionId}`, this.getAxiosConfig());
    ...
    async getOrgMetadata() {
        try {
            const response = await axios.get(`${this.baseUrl}/api/v1/org`, this.getAxiosConfig());
            return response.data;
        }
        catch (error) {
    ...
    async getCurrentUser() {
        try {
            const response = await axios.get(`${this.baseUrl}/api/v1/users/me`, this.getAxiosConfig());
            return {
                id: response.data.id,
     ...
     */
    async setAgentOwnersStandard(agentId, orgId, userId) {
        try {
            ...
            const response = await axios.post(`${this.baseUrl}/governance/api/v1/resource-owners`, {
     ...
     */
    async setAgentOwnersDeveloper(agentId, orgId) {
        try {
            // Step 1: Setup proxy
            const setupResponse = await axios.post(`${this.baseUrl}/devtools/api/ai-agent/ramp/setupProxy?orgId=${orgId}`, {}, this.getAxiosConfig());
            ...
            const resourceOrn = `orn:okta:directory:${orgId}:workload-principals:ai-agents:${agentId}`;
            const ownersResponse = await axios.put(`${this.baseUrl}/devtools/api/ai-agent/ramp/resourceOwners/${encodeURIComponent(resourceOrn)}`, {}, this.getAxiosConfig());
     ...
     */
    async removeAgentOwnersStandard(agentId, orgId) {
        try {
            const resourceOrn = `orn:okta:directory:${orgId}:workload-principals:ai-agents:${agentId}`;
            const response = await axios.post(`${this.baseUrl}/governance/api/v1/resource-owners`, {
                principalOrns: null,
     ...
     */
    async removeAgentOwnersDeveloper(orgId) {
        try {
            const response = await axios.delete(`${this.baseUrl}/devtools/api/ai-agent/ramp/setupProxy?orgId=${orgId}`, this.getAxiosConfig());
 ...
 */
export async function convertPublicKeyToJWK(publicKeyPem) {
 ...
 */
export function constructAuthServerORN(orgId, authServerId) {
