 ...
 */
function createClientAssertion(clientId, audience, privateKeyPem, kid) {
 ...
 */
async function validateMcpAS(env) {
    try {
 ...
 */
async function validatePrivateKey(env) {
    try {
...
async function validateEnvFiles() {
    try {
        ...
        const missing = [];
        if (!fs.existsSync(agent0AppEnvPath))
            missing.push(agent0AppEnvPath);
        if (!fs.existsSync(agent0AgentEnvPath))
            missing.push(agent0AgentEnvPath);
        if (!fs.existsSync(todo0AppEnvPath))
            missing.push(todo0AppEnvPath);
        if (!fs.existsSync(todo0McpEnvPath))
            missing.push(todo0McpEnvPath);
        if (missing.length > 0) {
            return {
                passed: false,
        ...
        requiredAgent0App.forEach((key) => {
            if (!agent0AppEnv[key])
                missingVars.push(`agent0 (.env.app): ${key}`);
        });
        requiredAgent0Agent.forEach((key) => {
            if (!agent0AgentEnv[key])
                missingVars.push(`agent0 (.env.agent): ${key}`);
        });
        requiredTodo0App.forEach((key) => {
            if (!todo0AppEnv[key])
                missingVars.push(`todo0 (.env.app): ${key}`);
        });
        requiredTodo0Mcp.forEach((key) => {
            if (!todo0McpEnv[key])
                missingVars.push(`todo0 (.env.mcp): ${key}`);
        });
