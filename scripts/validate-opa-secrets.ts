...
import { createOPAClientFromCredentials, type OPAClient } from './lib/opa-api.js';
...
interface OPAEnvConfig {
  OPA_BASE_URL?: string;
  OPA_TEAM_NAME?: string;
  OPA_KEY_ID?: string;
  OPA_KEY_SECRET?: string;
  OPA_RESOURCE_GROUP_ID?: string;
  OPA_PROJECT_ID?: string;
  OPA_FOLDER_ID?: string;
  OPA_LLM_PROVIDER?: string;
  OPA_ANTHROPIC_SECRET_NAME?: string;
  OPA_ANTHROPIC_MODEL_SECRET_NAME?: string;
 ...
 */
function loadEnvFile(filePath: string): OPAEnvConfig {
 ...
 */
function maskSecret(value: string | undefined, showChars: number = 8): string {
 ...
 */
async function validateEnvFile(): Promise<ValidationResult> {
 ...
 */
async function validateOPAConnection(env: OPAEnvConfig): Promise<ValidationResult> {
 ...
 */
async function validateSecretsAccess(env: OPAEnvConfig, client: OPAClient): Promise<ValidationResult> {
 ...
 */
async function validateSecretRetrieval(
  env: OPAEnvConfig,
  client: OPAClient,
  secretName: string
...
async function validate(): Promise<void> {
  ...
  let client: OPAClient | null = null;
  let passedCount = 0;
  let failedCount = 0;
  ...
  if (connResult.passed) {
    ...
    client = await createOPAClientFromCredentials(
      env.OPA_BASE_URL!,
      env.OPA_TEAM_NAME!,
      env.OPA_KEY_ID!,
      env.OPA_KEY_SECRET!
